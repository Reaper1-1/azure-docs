...
**Backups between vaults** | Backup is within a region.<br/><br/> You need a vault in every Azure region that contains resources you want to back up. You can't back up to a different region.<br/><br/> Cross-subscription support depends on the workload type. For Azure Files, it is supported. For Azure VMs, cross-subscription backup is supported (Preview). Learn more about support and limitations in the [Azure VM backup support matrix](backup-support-matrix-iaas.md#supported-backup-actions).
