...
Back up the VM to a different region | Not Supported.
Back up the VM to a different subscription | Supported (Preview) <br><br> - Not supported for encrypted (including ADE) and Confidential VMs. <br> - Snapshot-tier restore points can be restored only to the virtual machine's subscription. <br> - Replacing an existing virtual machine during restore is supported only in the virtual machine's subscription. <br> - Moving a Recovery Services vault that contains cross-subscription protected virtual machines isn't supported. <br> - Backup Move isn't supported for virtual machines protected across subscriptions.<br>
